...
function renderList(el: HTMLElement, depth: number): string {
	...
	return items.map((item, index) => {
		...
		const content = renderBlockChildren(item, depth + 1).trim() || collapseWhitespace(item.textContent ?? "");
...
function renderNode(node: Node, depth: number): string {
	...
	if (tag === "li") return renderBlockChildren(el, depth);
	if (tag === "blockquote") {
	...
	if (tag === "p" || tag === "div" || tag === "section" || tag === "article") {
		return renderBlockChildren(el, depth) || renderInlineChildren(el);
	}
