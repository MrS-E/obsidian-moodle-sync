...
const COMPLEX_HTML_SELECTORS = "table, details, summary, audio, video, svg, math";

export function renderMoodleHtml(html: string): string {
	const doc = decodeHtml(html);
	sanitizeMoodleHtml(doc);
	...
	return doc.body.querySelector(COMPLEX_HTML_SELECTORS)
		? `${markdown}\n\n${safeHtml}`.trim()
		: markdown;
}

function sanitizeMoodleHtml(doc: Document): void {
	doc.querySelectorAll("script, style, iframe, object, embed, form, input, button").forEach(element => element.remove());
	doc.querySelectorAll("textarea").forEach(element => {
		const replacement = doc.createElement("div");
		replacement.className = "moodle-text-answer";
